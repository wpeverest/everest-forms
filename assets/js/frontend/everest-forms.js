...
jQuery(function ($) {
	...
	var everest_forms = {
		...
		init_mailcheck: function () {
				...
				$el.mailcheck({
					suggested: function (el, suggestion) {
						...
						if (el.parents("span.input-wrapper").length) {
							$(el)
								.parents("span.input-wrapper")
								.after(
									'<label class="evf-error mailcheck-error" id="' +
										id +
										'_suggestion">' +
										suggestion_msg +
										"</label>"
			...
			$(document).on(
				"click",
				".evf-field-email .mailcheck-suggestion",
				function (e) {
					var $el = $(this),
						id = $el.attr("data-id");
					e.preventDefault();
		...
		init_datepicker: function () {
			...
			if (
				evfDateField.length &&
				evfDateField.find(".flatpickr-field").length
			) {
				evfDateField.find(".flatpickr-field").each(function () {
					...
					if (inputData.minDateRange) {
						...
						if ("today" === inputData.minDateRange) {
							...
							/^\s*[-+]?\d+\s*d/i.test(inputData.minDateRange)
						) {
							minDateRange = inputData.minDateRange
								.match(/^\s*[-+]?\d+\s*d/i)[0]
								.replace("d", "");
					...
					if (inputData.maxDateRange) {
						...
						if ("today" === inputData.maxDateRange) {
							...
							/^\s*[-+]?\d+\s*d/i.test(inputData.maxDateRange)
						) {
							maxDateRange = inputData.maxDateRange
								.match(/^\s*[-+]?\d+\s*d/i)[0]
								.replace("d", "");
			...
			$(document)
				.find(".evf-field-date-time input")
				.on("change", function (event) {
					...
					if (slotBooking === 1) {
						var dataTimeValue = $(this).val(),
							...
							data = {
								action: "everest_forms_slot_booking",
								"data-time-value": dataTimeValue,
								"data-format": dateFormat,
								"data-time-format": dateTimeFormat,
								mode: mode,
								"form-id": form_id,
								"time-interval": time_interval,
								security:
									everest_forms_params.everest_forms_slot_booking,
							};
		...
		change_minutes: function ($this) {
			...
			var id = $this.attr("id");
			if (
				typeof $this.siblings("#minute-select-" + id).attr("id") !=
				"undefined"
			) {
				...
				for (var i = 0; i <= 59; i++) {
					if (
						$this.siblings("#hour-select-" + id).val() ==
							min_hour &&
						i < min_minute
					) {
						continue;
		...
		format_dropdown_date: function ($this) {
			...
			var formatted_date = "";
			if (
				setting.date_time == "date" ||
				setting.date_time == "date-time"
			) {
			...
			$.validator.addMethod(
				...
				function (value, element) {
					var $ul = $(element).closest("ul"),
						...
						choiceLimit = parseInt(
							$ul.attr("data-choice-limit") || 0,
							10
						);
			...
			this.$everest_form.each(function () {
				...
				$(".evf-field").each(function () {
					...
					if ($(this).is(".evf-field-payment-single")) {
						if (
							!$(this)
								.find(".evf-payment-price")
								.is(".evf-payment-user-input")
						) {
							$(this)
								.find(".evf-payment-price")
								.attr("required", false);
						...
						$(this).is(
							".evf-field-checkbox, .evf-field-payment-checkbox"
						)
					) {
						key = key + "[]";
						...
						$(this).is(".evf-field-email") ||
						$(this).is(".evf-field-password")
					) {
						...
						var sub_field_error_messages = {
							address1: $(this).data(
								"required-field-message-address1"
							),
							city: $(this).data("required-field-message-city"),
							state: $(this).data("required-field-message-state"),
							postal: $(this).data(
								"required-field-message-postal"
							),
							country: $(this).data(
								"required-field-message-country"
							),
						};

						var sub_field_types = Object.keys(
						...
						if (row_keys && Array.isArray(row_keys)) {
							for (var i = 0; i < row_keys.length; i++) {
								...
								sub_field_error_messages[row_key] = $(
									this
								).data("required-field-message-" + row_key);
							}
							for (var i = 0; i < row_keys.length; i++) {
								error_message =
									sub_field_error_messages[row_keys[i]];
								key =
									"everest_forms[form_fields][" +
									field_id +
									"][" +
				...
				$this.validate({
					...
					errorPlacement: function (error, element) {
						if (
							element
								.closest(".evf-field")
								.is(".evf-field-privacy-policy")
						) {
							element.closest(".evf-field").append(error);
						} else if (
							element
								.closest(".evf-field")
								.is(".evf-field-range-slider")
						) {
							if (
								element
									.closest(".evf-field")
									.find(".evf-field-description").length
							) {
								element
									.closest(".evf-field")
									.find(".evf-field-description")
									.before(error);
							} else {
								element.closest(".evf-field").append(error);
							}
							...
							element
								.closest(".evf-field")
								.is(".evf-field-scale-rating")
						) {
							element
								.closest(".evf-field")
								.find(".everest-forms-field-scale-rating")
								.after(error);
						} else if (
							...
							if (
								element.hasClass(
									"everest-forms-likert-field-option"
								)
							) {
								element
									.closest("tr")
								...
								element
									.closest(".evf-field-checkbox")
									.find("label.evf-error")
									.remove();
								element
									.parent()
									.parent()
							...
							if (
								element.parent().find("label.evf-error:visible")
									.length === 0
							) {
								element
									.parent()
							...
							element.is("select") &&
							element.hasClass("evf-enhanced-select")
						) {
							if (
								element.parent().find("label.evf-error:visible")
									.length === 0
							...
							element.hasClass("evf-smart-phone-field") ||
							element.hasClass(
							...
							if (element.parents("span.input-wrapper").length) {
								...
								element.attr("data-rule-evf-captcha") ===
									"image"
							) {
								error.insertAfter(
									element.siblings(".captcha-icon-wrap")
								);
							} else {
					...
					highlight: function (element, errorClass, validClass) {
						...
						$parent
							.removeClass("everest-forms-validated")
							.addClass("everest-forms-invalid evf-has-error");
					},
					...
					submitHandler: function (form) {
						...
						var razorpayForms = $form.find(
							"[data-gateway='razorpay']"
						);
						var stripeForms = $form.find(
							"[data-gateway*='stripe']"
						);
						// Process form.
						if (processText) {
						...
						if (
							1 !== $form.data("ajax_submission") ||
							(stripeForms.length < 0 &&
								0 !== stripeForms.children.length)
						) {
							form.submit();
						} else {
							return;
					...
					onkeyup: function (element, event) {
						...
						if (
							$(element).hasClass(
								"everest-forms-novalidate-onkeyup"
							)
						) {
							return;
						}
		...
		validate_field: function (e) {
			...
			if ($body.hasClass("everest-forms-is-offline")) {
				$parent.removeClass(
					"everest-forms-invalid everest-forms-invalid-required-field everest-forms-invalid-email everest-forms-validated"
				);
			} else if (
				$parent.hasClass("evf-field-address") ||
				$parent.hasClass("evf-field-payment-single") ||
				$("body").hasClass("everest-forms-is-offline")
			) {
				if (0 === $parent.find("input.evf-error").length) {
					$parent
						.removeClass(
							"everest-forms-invalid everest-forms-invalid-required-field everest-forms-invalid-email"
						)
						.addClass("everest-forms-validated");
				}
			} else {
				if ("input" === event_type) {
					$parent.removeClass(
				...
				if ("validate" === event_type || "change" === event_type) {
					if (validate_required) {
						if (
							$this.hasClass("everest-forms-likert-field-option")
						) {
							if ($parent.find("input.evf-error").length > 0) {
								$parent
									.removeClass("everest-forms-validated")
									.addClass(
										"everest-forms-invalid everest-forms-invalid-required-field"
									);
								validated = false;
							}
						} else if (
						...
						if ($this.val()) {
							...
							if (!pattern.test($this.val())) {
								$parent
									.removeClass("everest-forms-validated")
									.addClass(
										"everest-forms-invalid everest-forms-invalid-email"
									);
								validated = false;
							}
						}
		...
		submission_scroll: function () {
			if ($("div.everest-forms-submission-scroll").length) {
				$("html,body").animate(
					{
						scrollTop:
							$("div.everest-forms-submission-scroll").offset()
								.top - 100,
					},
					1000
				);
		...
		init_enhanced_select: function () {
			...
			try {
				$(document.body)
					.on("evf-frontend-enhanced-select-init", function () {
						if ("undefined" !== typeof $.fn.selectWoo) {
							$("select.evf-enhanced-select:visible")
								.filter(":not(.evf-enhanced)")
								.each(function () {
		...
		checkUncheckAllcheckbox: function () {
			...
			var all_select_all_chk = $("form.everest-form")
				.find(".evf-field, .evf-field-checkbox, .form-row")
				.find("ul")
				.find("li.evf-select-all-checkbox-li")
				.find("#evfCheckAll");
			...
			Array.prototype.slice
				.call(
					document.querySelectorAll(
						".everest-forms-min-words-length-enabled"
					)
				)
				.map(function (event) {
					var minWords =
						parseInt(event.dataset.textMinLength, 10) || 0;
