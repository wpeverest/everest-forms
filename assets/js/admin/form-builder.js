	...
	var EVFPanelBuilder = {
		...
		$( document ).find( ".everest-forms-col-width-container input[type='number']" ).on( 'change keyup', function() {
			...
			if ( gridType === '2' ) {
				...
				if (nextColContainer.length > 0) {
					let nextColGridId = nextColContainer.find("input[type='number']").data('grid-id');
					let totalOtherColsWidth = 0;

					$adminRow.find('.evf-admin-grid').each(function() {
						let $col = $(this);
						let $orgGridId = $col.data('grid-id');

						if ($orgGridId !== gridId && $orgGridId !== nextColGridId) {
							let colWidth = $col[0].style.width ? parseFloat($col[0].style.width) : 33.33;
							totalOtherColsWidth += colWidth;
						}
					});

					let nextColWidth = 100 - value - totalOtherColsWidth;
					...
					$(document)
						.find('.evf-admin-row[data-row-id="' + rowId + '"] .evf-admin-grid[data-grid-id="' + nextColGridId + '"]')
						.css('width', nextColWidth + '%');

					$(document)
						.find('.evf-admin-row[data-row-id="' + rowId + '"] .evf-admin-grid[data-grid-id="' + gridId + '"]')
						.css('width', value + '%');
					...
					$(document)
						.find('.evf-admin-row[data-row-id="' + rowId + '"] .evf-admin-grid[data-grid-id="' + prevColGridId + '"]')
						.css('width', prevColWidth + '%');
		...
		$( document ).on('change', '.everest-forms-col-option-auto-width input[type="checkbox"]', function() {
			let $el 	  		= $( this ),
				...
				rowId			= $el.data( 'row-id' ),
				originalWidth 	= $( this ).data( 'original-col-width' );

				let $inputEl = $el.closest('.everest-forms-col-option-row')
                  .find('.everest-forms-col-width-container input[type="number"]');
		...
		bindChooseRowType : function () {
			$( '.evf-add-row-content svg' ).on( 'click', function( e ){
					e.preventDefault();
					$( '#evf-select-row-type-outer-wrapper' ).removeClass( 'everest-forms-hidden' );
					$( this ).closest( '.evf-add-row-content' ).hide();

					$( document ).on( 'click', '.evf-add-row-cancel-btn span svg', function( e ){
						e.preventDefault();
						$( '#evf-select-row-type-outer-wrapper' ).addClass( 'everest-forms-hidden' );
						$( '.evf-add-row-content' ).show();
						EVFPanelBuilder.bindFields();
					});
				});
