	...
	var EVFPanelBuilder = {
		...
		bindAddNewRow: function () {
			$('body').on('click', '.evf-add-row span', function () {
				...
				var last_row = wrapper.find('.evf-admin-row').last();
				var row_clone = $(
					'<div class="evf-admin-row ui-sortable-handle" data-row-id="' +
						max_row_id +
						'"></div>',
				);
				...
				last_row.find('.evf-admin-grid').each(function () {
					$(this)
						.children('*')
						.each(function () {
				...
				var grid_selector = last_row.find('.evf-grid-selector.active');
				var grid_id = 1;

				if (grid_selector.length > 0) {
					grid_id = parseInt(grid_selector.attr('data-evf-grid'), 10);
				}
				...
				if (
					...
					row_clone.append(last_row.find('.evf-toggle-row').clone());
					row_clone.append('<div class="clear evf-clear"></div>');

					var grid_lists = $('<div class="evf-grid-lists"></div>');
					var grid_node = $(
					...
					for (var $grid_number = 1; $grid_number <= grid_id; $grid_number++) {
						var new_grid = grid_node.clone();
						new_grid.attr('data-grid-id', $grid_number);
						grid_lists.append(new_grid);
					}

					grid_lists.find('.evf-admin-grid').eq(0).append(grids.html());

					row_clone.append(grid_lists);
					row_clone.append('<div class="clear evf-clear"></div>');
					...
					var row_clone_simple = last_row.clone();
					row_clone_simple.attr('data-row-id', max_row_id);
					wrapper.append(row_clone_simple);
					EVFPanelBuilder.bindFields();
					EVFPanelBuilder.checkEmptyGrid();
					$this.trigger('everest-forms-after-add-row', row_clone_simple);
				}
			});
		},
		...
		bindGridSwitcher: function () {
			...
			$('body').on('click', '.evf-grid-selector', function () {
				...
				var $grid_lists = $this_single_row.find('.evf-grid-lists');

				$grid_lists.find('.evf-admin-grid').each(function () {
					$(this)
						.children('*')
						.each(function () {
							grids.append($(this).clone());
				...
				$grid_lists.find('.evf-admin-grid').remove();
				$grid_lists.find('.evf-clear').remove();
				...
				$grid_lists.append('<div class="clear evf-clear"></div>');
				$grid_lists.find('.evf-admin-grid').eq(0).append(grids.html());
