<?php
/**
 * The notification model class for ThemeGrill SDK
 *
...
/**
 * Notification module for ThemeGrill SDK.
 */
class Notification extends AbstractModule {
	...
	private static function set_last_active_notification_timestamp() {
		...
		update_option( 'themeGrill_sdk_notifications', $metadata );
	}
	...
	public static function get_notification_html( $notification_details ) {
		...
		$notification_html                              = '<div class="notice notice-' . $type . ' is-dismissible themegrill-sdk-notice" data-notification-id="' . esc_attr( $notification_details['id'] ) . '" id="' . esc_attr( $notification_details['id'] ) . '-notification"> <div class="themegrill-sdk-notification-box">';
	...
	public static function render_snippets() {
		...
		?>
		<style type="text/css">
			.themegrill-sdk-notification-box {
				padding: 3px;
			}

			.themegrill-sdk-notification-box .wrap-flex {
				display: flex;
				align-items: center;
			...
			.themegrill-sdk-notification-box .wrap-flex img {
				width: 42px;
				object-fit: cover;
			}

			.themegrill-sdk-notification-box .actions {
				margin-top: 6px;
				margin-bottom: 4px;
			}

			.themegrill-sdk-notification-box .button {
				margin-right: 5px;
			}
		</style>
		<script type="text/javascript">
			(function ($) {
				$(document).ready(function () {
					$('#wpbody-content').on('click', ".themegrill-sdk-notice a.button, .themegrill-sdk-notice .notice-dismiss", function (e) {

						var container = $('.themegrill-sdk-notice');
						var link = $(this);
						...
						$.post(
							ajaxurl,
							{
								...
								'action': 'themeGrill_sdk_dismiss_notice',
								'id': notification_id,
								'confirm': confirm,
							},
						).fail(function() {
						...
						if (confirm === 'yes') {
							$(this).trigger('themegrill-sdk:confirmed');
						} else {
							$(this).trigger('themegrill-sdk:canceled');
						}
						container.hide();
	...
	public static function setup_notifications() {
		$notifications       = apply_filters( 'themeGrill_sdk_registered_notifications', [] );
		$notifications       = array_filter(
			$notifications,
			function ( $value ) {
				if ( ! isset( $value['id'] ) ) {
	...
	public function load( $product ) {
		if ( apply_filters( 'themeGrill_sdk_hide_notifications', false ) ) {
			return;
		}
		...
		add_action( 'wp_ajax_themeGrill_sdk_dismiss_notice', array( __CLASS__, 'dismiss' ) );
