...
const SiteAssistant:React.FC<Props> = ({siteAssistantQuery}) => {
  ...
  const skipSpamProtectionMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description: error?.message || __("Failed to skip spam protection setup.", "everest-forms"),
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  const sendTestEmailMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        title: __("Error", "everest-forms"),
        description: error?.message || __("Failed to send test email.", "everest-forms"),
        status: "error",
        duration: 3000,
        isClosable: true,
      });
    ...
    >
      ...
      <Collapse in={open?.sendTestEmail}>
        <Stack gap={5}>
          ...
          <FormControl gap={2}>
            <FormLabel>{__("Email Address (To send test email to)", 'everest-forms')}</FormLabel>
            <Input
        ...
        <Stack gap={5}>
          ...
          >
            ...
            <Link
              ...
              pointerEvents={skipSpamProtectionMutation.isLoading ? "none" : "auto"}
            >
  ...
  const stepsConfig: StepConfig[] = useMemo(() => [
    ...
    },
    {
      ...
      isCompleted: (data) => !!data?.skipped_steps?.includes('spam_protection'),
      renderContent: renderSpamProtectionContent,
    },
  ], [open, testEmail, sendTestEmailMutation.isLoading, skipSpamProtectionMutation.isLoading, siteData]);
