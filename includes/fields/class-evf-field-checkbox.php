	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		foreach ( $choices as $choice ) {
			...
			if ( ! empty( $field['choices_images'] ) ) {
				...
				printf( '<input type="checkbox" %s %s %s %s>', evf_html_attributes( $choice['id'], $choice['class'], $choice['data'], $choice['attr'] ), esc_attr( $choice['required'] ), esc_attr( isset( $field['readonly'] ) ? 'disabled' : '' ), checked( '1', $choice['default'], false ) );
