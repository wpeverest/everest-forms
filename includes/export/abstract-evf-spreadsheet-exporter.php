...
/**
 * EVF_Spreadsheet_Exporter Class.
 */
abstract class EVF_Spreadsheet_Exporter {
	 ...
	 */
	public function export( $content_type ) {
		$this->prepare_data_to_export();
		$this->send_headers( $content_type );
		$this->send_content( chr( 239 ) . chr( 187 ) . chr( 191 ) . $this->export_column_headers() . $this->get_spreadsheet_data() );
		die();
	}
	 ...
	 * @param  string $content_type Content-Type.
	 */
	public function send_headers( $content_type ) {
	 ...
	 */
	public function set_filename( $filename, $ext ) {
		$this->filename = sanitize_file_name( str_replace( $ext, '', $filename ) . $ext );
	}
	 ...
	 *
	 * @param string $spreadsheet_data All Spreadsheet content.
	 */
	public function send_content( $spreadsheet_data ) {
		echo $spreadsheet_data; // @codingStandardsIgnoreLine
	}

	/**
	 * Get Spreadsheet data for this export.
	 *
	 * @return string
	 */
	protected function get_spreadsheet_data() {
	...
	/**
	 * Export rows in Spreadsheet format.
	 *
	 * @return string
	 */
	...
	/**
	 * Export rows to an array ready for the Spreadsheet.
	 *
	...
	/**
	 * Escape a string to be used in a Spreadsheet context
	 *
	 * Malicious input can inject formulas into Spreadsheet files, opening up the possibility
	 ...
	 *
	 * @param string $data Spreadsheet field to escape.
	 * @return string
	 */
	...
	/**
	 * Format and escape data ready for the Spreadsheet file.
	 *
	...
	/**
	 * Implode Spreadsheet cell values using commas by default, and wrapping values
	...
	/**
	 * Write to the Spreadsheet file, ensuring escaping works across versions of
	 * PHP.
	 *
