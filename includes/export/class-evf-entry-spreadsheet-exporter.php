<?php
/**
 * Handles entry Spreadsheet export.
 *
 ...
 */
if ( ! class_exists( 'EVF_Spreadsheet_Exporter', false ) ) {
	require_once EVF_ABSPATH . 'includes/export/abstract-evf-spreadsheet-exporter.php';
}

/**
 * EVF_Entry_Spreadsheet_Exporter Class.
 */
class EVF_Entry_Spreadsheet_Exporter extends EVF_Spreadsheet_Exporter {
	...
	/**
	 * Prepare and get quiz report data in Spreadsheet format.
	 */
	public function get_quiz_report() {
		...
		$respondent_score   = 0;
		$obtained_score     = 0;
		$include_all_fields = apply_filters( 'evf_include_all_fields_in_quiz_report_spreadsheet', false );

		// Add form fields in the Spreadsheet content.
		foreach ( $form_fields as $field_id => $field ) {
			...
			$columns[] = $this->sanitize_spreadsheet_cell_data( $field['label'] );
			$row[]     = $this->sanitize_spreadsheet_cell_data( $score );
		}
		...
		foreach ( $extra_data as $key => $value ) {
			$columns[] = $this->sanitize_spreadsheet_cell_data( $key );
			$row[]     = $this->sanitize_spreadsheet_cell_data( $value );
		}
	...
	/**
	 * Sanitize data for a cell in Spreadsheet.
	 *
	 * @param string $str Data to sanitize.
	 */
	public function sanitize_spreadsheet_cell_data( $str ) {
		$str = (string) $str;
		$str = str_replace( '"', '""', $str );
		$str = '"' . $str . '"';
		return $str;
	...
	protected function generate_row_data( $entry ) {
		...
		foreach ( $columns as $column_id => $column_name ) {
			...
			if ( isset( $entry->meta[ $column_id ] ) ) {
				...
				$value = apply_filters( 'everest_forms_html_field_value', $value, $entry->meta[ $column_id ], $entry, 'export-spreadsheet', $column_id );
			...
			$row[ $column_id ] = apply_filters( 'everest_forms_format_spreadsheet_field_data', preg_match( '/textarea/', $column_type ) ? sanitize_textarea_field( $value ) : sanitize_text_field( $value ), $raw_value, $column_id, $column_name, $columns, $entry );
		}
