...
<script type="text/html" id="tmpl-everest-forms-field-preview-choices">
	...
	<# } else { #>
		<ul class="widefat primary-input">
			<# _.each( data.order, function( choiceID, key ) {  #>
				<li>
					<input type="{{ data.type }}" disabled<# if ( 1 === data.settings.choices[choiceID].default ) { print( ' checked' ); } #>>{{{ data.settings.choices[choiceID].label + ' - '+ evf_data.currency_symbol + ' ' + data.amountFormat(evf_data, data.amountSanitize(evf_data, data.settings.choices[choiceID].value))}}}
				</li>
			<# }) #>
		</ul>
	<# } #>
