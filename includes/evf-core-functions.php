...
function evf_string_translation( $form_id, $field_id, $variable, $name_suffix = '' ) {
	$form_id  = isset( $form_id ) ? 'everest_forms_' . absint( $form_id ) : 0;
	$field_id = isset( $field_id ) ? $field_id . $name_suffix : '';

	if ( function_exists( 'icl_register_string' ) ) {
		icl_register_string( $form_id, $field_id, $variable );
	}

	if ( function_exists( 'icl_t' ) ) {
		$variable = icl_t( $form_id, $field_id, $variable );
	}
