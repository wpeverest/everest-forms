 ...
 */
function load_previous_field_value( $properties, $field, $form_data ) {

	if ( ! isset( $_POST['everest_forms'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
		return $properties;
	}
	$data = ! empty( $_POST['everest_forms']['form_fields'][ $field['id'] ] ) ? wp_unslash( $_POST['everest_forms']['form_fields'][ $field['id'] ] ) : array(); // phpcs:ignore WordPress.Security.NonceVerification
	...
	if ( 'checkbox' === $field['type'] ) {
		...
		$properties['inputs']['primary']['attr']['value'] = sanitize_text_field( wp_unslash( $_POST['everest_forms']['form_fields'][ $field['id'] ] ) );// phpcs:ignore WordPress.Security.NonceVerification
	}
	return $properties;
}
add_filter( 'everest_forms_field_properties', ( 'load_previous_field_value' ), 99, 3 );
