...
class EVF_Shortcode_Form {
	...
	public static function fields( $form_data, $title, $description ) {
		...
		foreach ( $structure as $row_key => $row ) {
			 ...
			 */
			$is_repeater = apply_filters( 'everest_forms_display_repeater_fields', $row, $form_data, true );
			...
			echo '<div class="evf-frontend-row" data-row="' . esc_attr( $row_key ) . '"' . ( ! is_array( $is_repeater ) ? $is_repeater : '' ) . '>'; // @codingStandardsIgnoreLine
