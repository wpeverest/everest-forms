	...
	public function field_element( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			...
			case 'text':
				...
				$output = sprintf( '%s<input type="%s" class="widefat %s" id="everest-forms-field-option-%s-%s" name="form_fields[%s][%s]" value="%s" placeholder="%s" %s %s>', $before, $type, $class, $id, $slug, $id, $slug, esc_attr( $args['value'] ), $placeholder, $data, ( $args['slug'] === 'meta-key' ? ' readonly' : '' ) );
				break;
